...
# Get HTTP parameter, ctid (caretaker id) and sid (swimmer id)
form = cgi.FieldStorage()
dcode = form.getfirst('dcode')

print('<h3>Department CSCI has the following faculty members.</h3>')
print('<ol>')
...
##query='''
##SELECT CONCAT(f.fname,' ',f.lname) AS faculty, IF(t2.advisees IS NULL,'', t2.advisees) AS advisees
##FROM faculty AS f LEFT JOIN
##    (SELECT s.advisor AS facId, GROUP_CONCAT(CONCAT('<li>',s.fname,' ',s.lname,'</li>')
